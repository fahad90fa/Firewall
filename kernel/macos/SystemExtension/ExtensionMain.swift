...
//
//  A Network Extension provider does not have a `main`. The system loads the
//  bundle, reads NEProviderClasses from Info.plist, instantiates the named
//  class, and drives it through the NEFilterDataProvider callbacks. This file
//  exists to start the run loop the provider's asynchronous work needs, and to
//  do the two things that must happen before any flow is judged.
//
//  # Order matters here
//
