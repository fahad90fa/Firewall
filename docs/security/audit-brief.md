...
| New behavioral defence layers: port-scan/sweep + DNS-tunnel detection | `daemon/src/logging/portscan.rs` (live-wired), `daemon/src/logging/dns_exfil.rs` | Reconnaissance and exfiltration shapes a per-flow decision can't see. Bounded state; each ships unit tests that double as a small labeled corpus. |
