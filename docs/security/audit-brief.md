...
| --- | --- | --- | --- |
| 1 | nftables emission — can a policy compile to a ruleset that allows what it names as denied, or vice-versa? | `cli/` packet compiler; model tests in `cli/tests/`; **real-packet conformance** in `tests/scenarios/enforcement_conformance.rs` | This is the layer that actually enforces today. A miscompile is a silent policy bypass — and the emitted ruleset is now loaded into a real kernel in a netns and hit with real packets, which is how the `allow-inspect` bypass would now be caught. Confirm the conformance harness cannot pass vacuously (it is `UFW_NFT_REQUIRE`/`UFW_NETNS_REQUIRE`-gated). |
...
| Detection efficacy is a measured number, not a slogan | `daemon/tests/waf_efficacy.rs` + `detection_efficacy.rs`, [`detection-efficacy.md`](../design/detection-efficacy.md) | **WAF full pipeline: 89.7% catch / 0% FP** on an independent web-attack corpus (all evasion variants caught); egress-anomaly 100%/0%; signature pre-filter 100% recall. Measured against known corpora, **not** a claim of novel-attack coverage. |
| Behavioral defence layers: port-scan/sweep, C2 beaconing, brute-force, DNS-tunnel | `daemon/src/logging/{portscan,beacon,bruteforce,dns_exfil}.rs` (first three live-wired) | Reconnaissance, C2 cadence, credential attacks, and exfiltration shapes a per-flow decision can't see. Bounded state; each ships unit tests that double as a small labeled corpus. Beacon/brute-force are triage alerts, not blocks. |
| Honeypot / deception layer | `cli/src/bin/ufw-nft/dashboard/honeypot.rs`, `policies/base/honeypot_decoys.yaml` | Console decoy routes + per-install canary honeytoken (near-zero FP) and passive nft decoy ports (`action: alert`). Opt-in auto-contain is **structurally** spoof-proof — it can only fire from the post-handshake HTTP handler, so a forged SYN can't reach it; public-only + rate-limited. Verify the canary is per-install (not the source constant) and that Tier-1 (spoofable) alerts never feed contain. |
| The live path survives sustained real traffic | `scripts/soak.sh` + `daemon/tests/soak.rs`, [`soak-results-latest.md`](../design/soak-results-latest.md), nightly `.github/workflows/soak.yml` | The latest data-path leak soak (6 min, 2.6M iterations, 9.8k RSS samples) came back `stable`, but that is a smoke, **not** the 30-day production run — a slope over minutes is warm-up-dominated. The 30-day fleet soak remains the dominant open gap. |
| The emitted ruleset actually enforces the policy against real packets | `tests/scenarios/enforcement_conformance.rs` (+ the netns harness) | Loads the emitted nft in a namespace and fires real loopback connections; allow must pass, deny must drop. Closes the class the `allow-inspect` bypass was in. |
| The network/fleet-facing Rust parsers do not crash on hostile bytes | `tests/scenarios/parser_fuzz.rs`, `cli/.../dashboard/mod.rs` fuzz test | Deterministic fuzzing of the wire-protocol decoder, the policy compiler, and the console HTTP head — the class the unauth `percent_decode` crash was in. |
| Enforcement changes are tamper-evident | `daemon/src/audit.rs`, `ufwd --verify-audit` | Hash-chained append-only log; documented as tamper-*evidence*, not proofing. |
| Fleet bundles can be public-key signed | `daemon/src/fleet.rs` (`--features tls`, `api.fleet_ed25519_pubkey`) | Ed25519 via `ring` replaces the shared-secret weakness; verify a valid-MAC-no-signature bundle is refused when a key is required. |
| The `.deb` is reproducible | `build/linux/verify-reproducible.sh` | Two builds byte-identical; a tampered build cannot hide behind non-determinism. |
