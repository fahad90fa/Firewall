...
| **Detection** (IDS/IPS sigs, WAF, egress anomaly, port-scan, DNS-tunnel, beaconing, JA3/JA4) | user-space daemon | Detects and logs; enforcement of a detection is a separate, opt-in response. WAF full-pipeline efficacy is **measured** (89.7% catch / 0% FP on an independent corpus) — see `docs/design/detection-efficacy.md`. |
